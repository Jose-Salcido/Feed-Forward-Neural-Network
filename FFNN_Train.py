...
def working_dir(test_path):
    ...
    try:
        ...
        print(f"\n{test_path} is not a valid directory.")
...
def create_parser():
    ...
    parser.add_argument('-o', '--output_dir', type = working_dir, required = False, default = ".",
...
def ScalingType(scaling_type):
    ...
    if scaling_type < 1 or scaling_type > 3:
        raise ValueError(f"\nScaling type {scaling_type} must be a whole number between 1 and 3.  Use 'python FFNN_Train.py -h' for help on usage.")
    else:
...
def ActivationFunction(activation_function):
    ...
    if activation_function < 1 or activation_function > 11:
        raise ValueError(f"\nActivation Funtion {activation_function} must be a whole number between 1 and 11. Use 'python FFNN_Train.py -h' for help on usage.")
    else:
...
def OptimizationFunction(optimization_function):
    ...
    if optimization_function < 1 or optimization_function > 7:
        raise ValueError(f"\nOptimization Funtion {optimization_function} must be a whole number between 1 and 7. Use 'python FFNN_Train.py -h' for help on usage.")
    else:
...
def LossFunction(loss_function):
    ...
    if loss_function < 1 or loss_function > 16:
        raise ValueError(f"\nLoss Funtion {loss_function} must be a whole number between 1 - 16. Use 'python FFNN_Train.py -h' for help.")
    else:
...
def generate_plots(
    ...
    plots = plot_functions.PlottingFunctions(output_path, dataframe, history, plot_label, x_truth_data, y_truth_data, y_predictions)
    logger.info('\n=========== GENERATING PLOTS ===========\n')
    plots.Histogram()
    ...
    logger.info(
        'R-squared explains percent of model variance, also known as coefficient of determination.')
    logger.info(
    ...
    logger.info('Probability value (p-value). This is the probability of obtaining test results at least as extreme as results actually observed.')
...
def train_neural_network(
    ...
    if not Path(input_file).exists():
        ...
        raise FileNotFoundError(f"\nThe CSV file {input_file} was not found in the provided directory.")
    else:
    ...
    output_h5_filename = "SurrogateModel_%dLayers_%dNodes_%dEpochs.h5" % (layers, nodes, epochs)
